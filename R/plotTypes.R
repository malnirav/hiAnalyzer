...
#### genome plot ####
#' @export
plot.CircleIdeogram <- function(sites, separateByCol="Alias", freeze="hg18", pointSize=.5, yAxisScatter=NULL, title="", clusterSitesBin=NULL, adjustTracks=TRUE, dbcon=NULL) {
    ...
    # get tracks #
    sites.gr <- as(makeRangedData(subset(sites,type=="insertion"),soloStart=TRUE,chromCol="Chr"),"GRanges")
    genes <- as(makeRangedData(unique(dbGetQuery(dbcon,paste("select * from ",freeze,".refflat",sep=""))[,3:6])),"GRanges")
    freeze.gr <- as(makeRangedData(unique(dbGetQuery(dbcon,paste("select chrom, 1 as start, size as end ,'*' as strand from ",freeze,".chrominfo",sep="")))),"GRanges")
    seqlengths(freeze.gr) <- end(freeze.gr)
    seqorder <- sapply(strsplit(gsub("chr","",seqlevels(freeze.gr)),"\\_"),"[[",1)    
    ...
    if(adjustTracks) {
        track.radius <- structure(seq(23,pmax(23/length(tracks),2),length=length(tracks)),names=tracks)
    } else {
    ...
    p <- ggplot() + layout_circle(freeze.gr, geom = "ideo", fill = "gray70", radius = 34, trackWidth = 2) 
    p <- p + layout_circle(freeze.gr, geom = "scale", size = 2, radius = 36, trackWidth = 2)
    p <- p + layout_circle(freeze.gr, geom = "text", aes(label = Chr), vjust = 0, radius = 38, trackWidth = 7)
    p <- p + layout_circle(genes.reduced, aes(y=logCount), geom = "bar", fill = "#FF0000", color = "red", radius = 28, trackWidth = 6, size=.2)
    
    # make a track per separateByCol from sites.gr #
    for(f in tracks) {
        dat <- subset(sites.gr,values(sites.gr)[,separateByCol]==f)
        if(asBars) {
        	p <- p + layout_circle(dat, aes(y=logCount), geom = "bar", fill = dataset[[f]], color = dataset[[f]], radius = track.radius[[f]], trackWidth = 6, size=.2)
        } else {
			...
			p <- p + layout_circle(dat, aes(y=YaxisScatter), size=pointSize, geom = "point", color = dataset[[f]], radius = track.radius[[f]])
        }
    }
    p <- p + theme(title=title)
    
    test <- qplot(data=data.frame(Track=factor(names(dataset),levels=names(dataset))),1,Track,fill=Track,geom="tile") + scale_fill_manual(values=dataset)
    tmp <- ggplot_gtable(ggplot_build(test))
    leg <- which(sapply(tmp$grobs, function(x) x$name) == "guide-box")
    legend <- tmp$grobs[[leg]]
    
    grid.arrange(p, legend,widths=unit.c(unit(1, "npc") - legend$width, legend$width), nrow=1)
}
...
plot.siteLogo <- function(seqs, samples, traditional=FALSE, ZeroCentered=TRUE, themes=NULL) {
    ...
    dat <- sapply(seqs,function(x) consensusMatrix(DNAStringSet(x),as.prob=T)[1:4,],simplify=F)
    if(ZeroCentered) {
    	dat <- lapply(dat, function(x) { newones <- seq(-(ncol(x)/2),(ncol(x)/2),by=1); colnames(x) <- newones[newones!=0]; x })
    }
    
    if(traditional) {
        sapply(names(dat), function(x) { quartz(x); seqLogo(makePWM(dat[[x]]) ) })       
    } else {
        melted <- do.call(rbind, lapply(names(dat), function(x) { cbind(samples=x, melt(cbind(as.data.frame(dat[[x]]),base=rownames(dat[[x]])),id="base")) }))
        ...
        p <- qplot(data=melted,variable,value,geom="line",colour=base,group=base,xlab="Position") + scale_y_continuous("Percent of Sequences",label=percent) + facet_wrap(~samples,ncol=1)
...
#' @export
plot.distToFeature <- function(dat, brks, byCol, annotCol, ratioMRC=FALSE, discreteBins=TRUE, themes=NULL, printPlotData=FALSE, bw=FALSE) {
    counts.df<-as.data.frame(xtabs(~type+get(byCol),dat,drop=T))
    names(counts.df)[2:3]<-c(byCol,"Total")
    plot.frame<-as.data.frame(xtabs(~cut(dat[,annotCol], brks,include.lowest=T, dig.lab=5)+get(byCol)+type, dat,drop=T))
    ...
    if(ratioMRC) {
        ...
        ratiosCalc<-ddply(plot.frame,~get(byCol)+annot+Dist.To.TSS,summarise,
        				  RatioMRC=Percent[tolower(type)=="insertion"]/Percent[tolower(type) %in% c("match","mrcs")])
        res <- droplevels(subset(merge(plot.frame,ratiosCalc,all.x=T), type=="insertion"))
        if(!discreteBins) {
            p <- qplot(data=res, Dist.To.TSS, RatioMRC, geom="bar", stat="identity", position="dodge", fill=get(byCol)) +
            scale_x_continuous("Distance",expand = c(0,0))
        } else {
            p <- qplot(data=res, Distance, RatioMRC, geom="bar", stat="identity", position="dodge", fill=get(byCol)) + 
            scale_x_discrete("Distance",expand = c(0,0))
        }
        p <- p + scale_y_continuous("Ratio Insertions/Matches",expand = c(0,0))
    } else {
        if(!discreteBins) {
            p <- qplot(data=plot.frame, Dist.To.TSS, Percent, geom="bar", stat="identity", position="dodge", fill=get(byCol)) +
             scale_x_continuous("Distance",expand = c(0,0))
        } else {
            p <- qplot(data=plot.frame, Distance, Percent, geom="bar", stat="identity", position="dodge", fill=get(byCol)) +
             scale_x_discrete("Distance",expand = c(0,0))
        }
        p <- p + scale_y_continuous("Percent of Sites",label=percent,expand = c(0,0)) + facet_wrap(~type,ncol=1)
    }
    
    if(!is.null(themes)) {
    	p <- p + get(themes)() + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) 	
    } else {
    ...
    if(bw) {
    	p <- p + scale_fill_grey(name=byCol, start=.3, end=.7)
    }
        
    if(printPlotData) { 
    	plot.frame$byCol <- plot.frame[,byCol]
    	print(cast(data=plot.frame, Distance~byCol, value='Percent', margins=TRUE, fun=sum))
    }
...
getColors <- function(n) {     
    allCols <- unique(c(brewer.pal(9,"Set1"), brewer.pal(8,"Dark2"), 
    					brewer.pal(8,"Accent"), brewer.pal(8,"Set2"), 
    					brewer.pal(12,"Paired"), brewer.pal(10,"Spectral")))
    if(n>length(allCols)) {
